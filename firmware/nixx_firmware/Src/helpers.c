...
void set_pwm(uint8_t pwm_arr[SPI_BUF_SIZE])
{
  ...
  if(pwm_arr[10] & 0x80)
  {
    htim1.Instance->CCR4 = pwm_arr[10] & 0x7f; // 0
    pwm_stats[10] = pwm_arr[10];
  }
  if(pwm_arr[1] & 0x80)
  {
    htim2.Instance->CCR2 = pwm_arr[1] & 0x7f; // 1
    pwm_stats[1] = pwm_arr[1];
  }
  if(pwm_arr[2] & 0x80)
  {
    htim2.Instance->CCR3 = pwm_arr[2] & 0x7f; // 2
    pwm_stats[2] = pwm_arr[2];
  }
  if(pwm_arr[3] & 0x80)
  {
    htim2.Instance->CCR4 = pwm_arr[3] & 0x7f; // 3
  ...
  if(pwm_arr[5] & 0x80)
  {
    htim2.Instance->CCR1 = pwm_arr[5] & 0x7f; // 5
    pwm_stats[5] = pwm_arr[5];
  }
  if(pwm_arr[6] & 0x80)
  {
    htim1.Instance->CCR3 = pwm_arr[6] & 0x7f; // 6
    pwm_stats[6] = pwm_arr[6];
  }
  if(pwm_arr[7] & 0x80)
  {
    htim3.Instance->CCR1 = pwm_arr[7] & 0x7f; // 7
    pwm_stats[7] = pwm_arr[7];
  }
  if(pwm_arr[8] & 0x80)
  {
    htim1.Instance->CCR1 = pwm_arr[8] & 0x7f; // 8
    pwm_stats[8] = pwm_arr[8];
  }
  if(pwm_arr[9] & 0x80)
  {
    htim1.Instance->CCR2 = pwm_arr[9] & 0x7f; // 9
    pwm_stats[9] = pwm_arr[9];
  }
  if(pwm_arr[11] & 0x80)
  {
    pwm_stats[11] = pwm_arr[11] & 0x7f; // left dot
    pwm_stats[11] > 125 ? pwm_stats[11] = 125 : pwm_stats[11];
  }
  if(pwm_arr[12] & 0x80)
  {
